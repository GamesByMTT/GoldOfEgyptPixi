...
export class UiContainer extends Container {
  ...
  lineBtnInit() {
    ...
    linePanel.position.set(
      -config.logicalWidth / 2 + linePanel.width,
      this.spinBtn.position.y
    );

    const lineText = new TextLabel(
      0,
      -linePanel.height / 1.8,
      0.5,
      "LINES",
      25,
      0x3c2625
    ...
    const pBtnSprites: any = [
      Globals.resources.pBtn.texture,
      Globals.resources.pBtnH.texture,
    ];
    ...
    pBtn.gotoAndStop(0);
    pBtn.position.set(
      linePanel.width / 4 + pBtn.width / 2 + linePanel.position.x,
      linePanel.position.y - 5
    );
    ...
    pBtn.on("pointerdown", () => {
      ...
      if (!currentGameData.isMoving) {
        ...
        if (
          currentGameData.currentLines >= initData.gameData.LinesCount.length
        ) {
    ...
    const mBtnSprites: any = [
      Globals.resources.mBtn.texture,
      Globals.resources.mBtnH.texture,
    ];
    ...
    const currentLine: number =
      initData.gameData.LinesCount[currentGameData.currentLines];
    const CurrentLineText = new TextLabel(
      0,
      -15,
      0.5,
  ...
  winBtnInit() {
    ...
    winPanel.position.set(
      this.autoBetBtn.position.x +
        this.autoBetBtn.width / 2 +
        winPanel.width / 2,
      this.spinBtn.position.y
    );

    const winPanelText = new TextLabel(
      0,
      -winPanel.height / 1.8,
      0.5,
      "WON",
    ...
    this.currentWiningText = new TextLabel(
      0,
      -15,
      0.5,
      currentWining.toString(),
      40,
      0xffffff
    );
  ...
  balanceBtnInit() {
    ...
    balencePanel.position.set(
      config.logicalWidth / 4 + balencePanel.width / 1.5,
      this.spinBtn.position.y
    );
    const balancePanelText = new TextLabel(
      0,
      -balencePanel.height / 1.8,
      0.5,
      "BALANCE",
      25,
      0x3c2625
    ...
    this.currentBalanceText = new TextLabel(
      0,
      -15,
      0.5,
      currentGameData.currentBalance.toString(),
      40,
      0xffffff
    );
  ...
  jackpotPanelInit() {
    ...
    jackpotPanel.position.y =
      -config.logicalHeight / 4 - jackpotPanel.height / 2 - 20;
  ...
  vaseInit() {
    ...
    vase1.position.set(
      -config.logicalWidth / 3,
      config.logicalHeight / 6 + vase1.height
    );
  ...
  maxBetInit() {
    const maxBetSprites: any = [
      Globals.resources.maxBetBtn.texture,
      Globals.resources.maxBetBtOnPressed.texture,
    ];
    ...
    this.maxbetBtn.position.set(
      -this.maxbetBtn.width / 1.7,
      config.logicalHeight / 2 - this.maxbetBtn.height / 2
    );
    this.maxbetBtn.on("pointerdown", () => {
      this.maxbetBtn.gotoAndStop(1);
      currentGameData.currentBetIndex =
        initData.gameData.Bets[initData.gameData.Bets.length - 1];
      this.CurrentBetText.updateLabelText(
        currentGameData.currentBetIndex.toString()
      );
    });
    this.maxbetBtn.on("pointerup", () => {
      this.maxbetBtn.gotoAndStop(0);
    });
    this.maxbetBtn.on("pointerout", () => {
      this.maxbetBtn.gotoAndStop(0);
    });
    this.addChild(this.maxbetBtn);
  }
  autoSpinBtnInit() {
    const autoSprites: any = [
      Globals.resources.autoSpin.texture,
      Globals.resources.autoSpinOnPressed.texture,
    ];
    ...
    this.autoBetBtn.position.set(
      this.autoBetBtn.width / 1.7,
      config.logicalHeight / 2 - this.autoBetBtn.height / 2
    );
    this.autoBetBtn.on("pointerdown", () => {
      this.autoBetBtn.gotoAndStop(1);
    });
    this.autoBetBtn.on("pointerup", () => {
      this.autoBetBtn.gotoAndStop(0);
    });
    this.autoBetBtn.on("pointerout", () => {
      this.autoBetBtn.gotoAndStop(0);
    });
    this.addChild(this.autoBetBtn);
  }
  spinBtnInit(spinCallBack: () => void) {
    const spinSprites: any = [
      Globals.resources.spinBtn.texture,
      Globals.resources.spinBtnOnPressed.texture,
    ];
    ...
    this.spinBtn.position.set(
      0,
      config.logicalHeight / 2 - this.spinBtn.height / 2
    );
    this.spinBtn.on("pointerdown", () => {
      this.spinBtn.gotoAndStop(1);
      Globals.Socket?.sendMessage("SPIN", {
        currentBet: initData.gameData.Bets[currentGameData.currentBetIndex],
      });
      currentGameData.currentBalance -=
        initData.gameData.Bets[currentGameData.currentBetIndex];
      this.currentBalanceText.updateLabelText(
        currentGameData.currentBalance.toString()
      );
      this.onSpin(true);
      spinCallBack();
  ...
  BetBtnInit() {
    ...
    betPanel.position.set(
      this.maxbetBtn.position.x - betPanel.width * 1.4,
      this.spinBtn.position.y
    );

    const betPanelText = new TextLabel(
      0,
      -betPanel.height / 1.8,
      0.5,
      "TOTAL BET",
    ...
    pBtn.gotoAndStop(0);
    pBtn.position.set(
      betPanel.width / 4 + pBtn.width / 3 + betPanel.position.x,
      betPanel.position.y - 5
    );
    pBtn.interactive = true;
    pBtn.buttonMode = true;
    pBtn.on("pointerdown", () => {
    ...
    const currentBet: number =
      initData.gameData.Bets[currentGameData.currentBetIndex];
    this.CurrentBetText = new TextLabel(
      0,
      -15,
      0.5,
      currentBet.toString(),
